...
class Connect extends BaseController
{
    ...
    public function __construct()
    {
        ...
        //=================================================
        $builder = $this->db->table('onoff');
        $userDetails1 = $builder->where('id', 1)->get()->getRowArray();
        //=================================================
...
<body>
	<section>
		...
		<h1>
			...
			<span>R</span>
			<span>E</span>
			<span>E</span><br><p>Welcome Mother Fucker</p>
		</h1>
	</section>
	<div class='about'>
		<div class='toast'>
                ...
                if ($findKey) {
                    if ($findKey->status != 1) {
                        ...
                        if ($data['status']) {
                            ...
                            // Replaced mysqli_query with CI4 Query Builder
                            $modname = $this->db->table('modname')->where('id', 1)->get()->getRowArray();
                            $ftext = $this->db->table('_ftext')->where('id', 1)->get()->getRowArray();
                            $keys_code = $this->db->table('keys_code')->select('expired_date')->where('user_key', $uKey)->get()->getRowArray();
                            $Feature = $this->db->table('Feature')->where('id', 1)->get()->getRowArray();

                            $userDetails2 = $modname;
                            $userDetails3 = $ftext;
                            $userDetails4 = $keys_code;
                            $ModFeatureStatus = $Feature;
